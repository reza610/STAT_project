...
#filtering the urban areas and setting the dates to date variables
urban_alaska <- Alaska_Covid_data %>% filter(Community == "Anchorage" | Community == "Fairbanks") %>% group_by(Date.Reported) %>% mutate(Total_Cases = sum(All.Cases)) %>% distinct(Date.Reported,.keep_all = TRUE)
urban_alaska <- urban_alaska %>% mutate(Date.Reported = as.Date(urban_alaska$Date.Reported, format = "%m/%d/%Y")) 
...
Alaska_CPI <- Alaska_CPI %>% mutate(Date.Reported = as.Date(as.yearmon(Alaska_CPI$Date.Reported, format = '%Y-%B')))
...
ggplot(Finland_data,aes(New_cases, Point.figure)) + geom_point()
summary(lm(Point.figure ~ New_cases, data = Finland_data))
```
...
```{r}
#Total CPI linear regression for US and India
US_total <- summary(lm(X7.Day.Avg.New.Cases.per.100K ~ All.items, data = All_Data))
India_total <- summary(lm(X7.Day.Avg.New.Cases.per.100K ~ India.All.items, data = All_Data))
